...
def embed_ResProp_data_in_pga(inputs):
    """Represent the res_prop residue data in PGA multivectors.

    Atoms (by atomic numbers) are represented as scalars, coordinates as trivectors.  
    They are summed (this is equivalent to concatenation, as an equi
    ...
    Parameters
    ----------
    inputs : torch.Tensor with shape (batchsize, objects, 4)
        res_prop residue data: a concatenation of atoms and coordinates
    ...
    # Build one multivector holding atoms and coordinates for each object
    atoms = inputs[:, :, [0]]                   # (batchsize, objects, 1)
    atoms = embed_scalar(atoms)                 # (batchsize, objects, 16)
    coordinates = inputs[:, :, 1:4]             # (batchsize, objects, 3)
    coordinates = embed_point(coordinates)      # (batchsize, objects, 16)
    multivector = atoms + coordinates           # (batchsize, objects, 16)
...
class ResPropGATrWrapper(BaseWrapper):
    ...
    def embed_into_ga(self, inputs):
        ...
        ----------
        inputs : torch.Tensor with shape (batchsize, objects, 4)
            rep_prop initial state: a concatenation of masses, initial positions, and initial
        ...
        multivector = embed_ResProp_data_in_pga(inputs)
...
class ResPropBaselineWrapper(nn.Module):
...
class ResPropSEGNNWrapper(nn.Module):
...
class ResPropSE3TransformerWrapper(nn.Module):
...
class ResPropGCANWrapper(nn.Module):
